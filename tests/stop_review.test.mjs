...
test('session group combines start and end as half a point each with lower score weight', () => {
    ...
    assert.equal(startOnly.score, 7);
    assert.equal(startOnly.gaps[1].done, 0.5);
    assert.equal(startOnly.gaps[1].total, 1);

    const complete = calculateJournalScore({
        now: new Date('2026-08-03T12:00:00Z'),
        journal: { '2026-08-03': { gross_pnl: 10, notes: 'Підсумок', sessionGoal: 'За планом', sessionReviewDone: true } },
    });
    assert.equal(complete.score, 8.5);
    assert.equal(complete.gaps[1].done, 1);
});

test('completed stop reviews contribute ten percent of the total journal score', () => {
    const journal = { '2026-08-03': { gross_pnl: 10, notes: 'Підсумок', sessionGoal: 'За планом', sessionReviewDone: true } };
...
test('learning is credited once per week and capped at four points per month', () => {
    const result = calculateJournalScore({
        ...
        learningDates: ['2026-08-03', '2026-08-05', '2026-08-12', '2026-08-20', '2026-08-27', '2026-08-28'],
    });
    const learning = result.gaps[3];
    assert.equal(learning.done, 4);
    assert.equal(learning.total, 4);
    assert.equal(learning.ratio, 1);
});
