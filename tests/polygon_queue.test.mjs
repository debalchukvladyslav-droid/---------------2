...
test('best-exit tickers open their exact journal trade and Polygon logs every ticker', async () => {
    ...
    assert.match(source, /data-best-exit-download/);
    assert.match(source, /data-best-exit-pause/);
    assert.match(source, /readPolygonDay/);
    assert.match(source, /немає свічок/);
    assert.doesNotMatch(source, /scheduleSilentRefresh/);
