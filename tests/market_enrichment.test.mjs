...
test('hybrid enrichment maps provider data and caches it', async () => {
    ...
    assert.deepEqual({ gap: result.gapPct, rvol: result.rvol, float: result.floatShares, atr: result.atr }, { gap: 10, rvol: null, float: 12500000, atr: null });
    assert.equal(result.provider, 'Finnhub'); const cached = await enrichMarketData('XYZ', options); assert.equal(cached.cached, true); assert.equal(calls, 3);
});
...
test('not-found, rate-limit, and invalid ticker errors stay actionable', async () => {
    ...
    await assert.rejects(() => enrichMarketData('XYZ', { fetchImpl: response(429), environment: { FINNHUB_API_KEY: 'f' }, skipCache: true }), (error) => error.code === 'MARKET_RATE_LIMITED');
