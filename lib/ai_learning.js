...
const VALID_ENTRY_SUBTYPES = new Set([
    'valid_entry', 'breakout_retest', 'pullback_entry', 'liquidity_sweep',
...
const REQUIRED_RESULT_CONTRACT = {
    ...
    replay: { decision: 'WAIT, ENTER or SKIP', entryZone: '', trigger: '', invalidation: '', reason: '' },
    signals: [],
};
...
export function buildCandidates(rows = [], profileContexts = new Map(), hashVersion = PROMPT_VERSION) {
    ...
    for (const row of rows) {
        ...
        trades.forEach((trade, index) => {
            ...
            screens.forEach((item) => usedScreenshotPaths.add(item.path));
        ...
        const profileContext = profileContexts.get(row.user_id) || {};
        const registry = profileContext.registry && typeof profileContext.registry === 'object' ? profileContext.registry : {};
        const tickerMap = profileContext.tickers && typeof profileContext.tickers === 'object' ? profileContext.tickers : {};
        allScreens(metrics).filter((screen) => !usedScreenshotPaths.has(screen.path)).forEach((screen, index, unmatched) => {
            const registered = registry[screen.path] || {};
            const ticker = compact(registered.ticker || tickerMap[screen.path], 24);
            const digest = crypto.createHash('sha256').update(String(screen.path)).digest('hex').slice(0, 20);
            const tradeKey = `${row.user_id}:${row.trade_date}:screenshot:${digest}`;
            const screenshotSet = [{
                path: screen.path,
                role: registered.screenshot_role || inferScreenshotRole(screen.path, index, unmatched.length),
                category: screen.category,
                matchMethod: 'unmatched_screenshot_audit',
                matchConfidence: 0,
                capturedAt: registered.captured_at || null,
                qualityStatus: registered.quality_status || 'unchecked',
                width: registered.pixel_width || null,
                height: registered.pixel_height || null,
            }];
            const snapshot = {
                tradeDate: row.trade_date, ticker, screenshotOnly: true,
                dayNotes: compact(row.notes || metrics.notes || metrics.comment),
                screenshotMatch: { method: 'unmatched_screenshot_audit', confidence: 0, ambiguous: true, alternatives: trades.length },
                screenshotSet,
            };
            const outcome = { pnl: null, kf: null, dayPnl: numberOrNull(row.pnl), dayKf: numberOrNull(row.kf) };
            output.push({
                user_id: row.user_id, journal_day_id: row.id, trade_date: row.trade_date,
                trade_key: tradeKey,
                content_hash: crypto.createHash('sha256').update(JSON.stringify({ promptVersion: hashVersion, tradeKey, snapshot, outcome, screenshots: screenshotSet })).digest('hex'),
                source_snapshot: snapshot, outcome, screenshot_path: screen.path,
            });
        });
    }
...
export function parseAiJson(text, validationContext = {}) {
    ...
    const replay = value.replay && typeof value.replay === 'object' ? value.replay : {};
    ...
    const result = {
        ...
        // Provider self-confidence is not a calibrated probability. Never expose certainty as 100%.
        ai_confidence: Math.max(0, Math.min(0.9, Number(value.confidence) || 0)),
        ...
        analysis_features: {
            ...
            replay: {
                decision: ['WAIT', 'ENTER', 'SKIP'].includes(String(replay.decision || '').toUpperCase()) ? String(replay.decision).toUpperCase() : 'WAIT',
                entryZone: compact(replay.entryZone, 80),
                trigger: compact(replay.trigger, 120),
                invalidation: compact(replay.invalidation, 120),
                reason: compact(replay.reason, 240),
            },
            signals,
        },
    };
...
async function storageImage(path, { cropRatio = 1 } = {}) {
    ...
    try {
        ...
        const safeCropRatio = Math.max(0.35, Math.min(1, Number(cropRatio) || 1));
        let outputMimeType = mimeType;
        if (safeCropRatio < 0.999 && dimensions.width && dimensions.height) {
            const cropWidth = Math.max(320, Math.min(dimensions.width, Math.floor(dimensions.width * safeCropRatio)));
            buffer = await sharp(buffer, { failOn: 'warning' })
                .extract({ left: 0, top: 0, width: cropWidth, height: dimensions.height })
                .png({ compressionLevel: 8 })
                .toBuffer();
            outputMimeType = 'image/png';
        }
        return { ok: true, status: dimensions.width ? 'ready' : 'ready_dimensions_unknown', path, bytes: buffer.length, ...dimensions, cropRatio: safeCropRatio, mime_type: outputMimeType, data: buffer.toString('base64') };
    } catch (error) {
...
async function storageImages(candidate) {
    ...
    const cropRatio = Number(candidate?.source_snapshot?.marketReplay?.cropRatio || 1);
    const loaded = await Promise.all(set.map(async (item) => ({ ...item, ...(await storageImage(item.path, { cropRatio })) })));
    return {
...
async function geminiGenerate(candidate, similar = [], { blind = false, repairAttempt = 0 } = {}) {
    ...
    const replayStage = candidate.source_snapshot?.marketReplay;
    const replayInstruction = replayStage
        ? `MARKET REPLAY: visible chart fraction ${Math.round(Number(replayStage.cropRatio || 1) * 100)}%, stage ${replayStage.stage}. You do not know future candles or outcome. Decide replay.decision as WAIT, ENTER, or SKIP. ENTER only when a concrete visible trigger exists now; otherwise WAIT. SKIP only when the setup is invalid, not merely incomplete. Describe entryZone without inventing an exact price, plus trigger and invalidation.\n`
        : '';
    const prompt = `Маніфест скріншотів: ${JSON.stringify(images.manifest)}\n${replayInstruction}${repairAttempt ? 'ПОПЕРЕДНЯ ВІДПОВІДЬ МАЛА НЕВАЛІДНИЙ JSON. Поверни повний синтаксично валідний об’єкт без markdown.\n' : ''}Ти виконуєш другий, поглиблений прохід аналізу входу трейдера. Використай накопичену пам'ять схожих угод, але перевіряй висновок за поточним скріном і журналом. Результат угоди є лише сигналом: плюс не доводить правильність, мінус не доводить помилку.
Окремо визнач:
...
export function shouldRunMemoryVerification(preliminary = {}, memory = []) {
    return isMemoryEligible(preliminary) && Array.isArray(memory) && memory.length > 0;
}
...
export function evaluateAnalysis(expectedPatternKey, analysis = {}) {
    ...
    return {
        ...
        compatibleMatch: semanticPatternMatch(expectedPatternKey, analysis.ai_pattern_key),
        evidenceComplete: Boolean(chartSummary && (visible.length || missing.length)),
        abstained,
    };
}

export function semanticPatternMatch(expectedPatternKey, predictedPatternKey) {
    if (expectedPatternKey === predictedPatternKey) return true;
    // A human may approve the broad valid-entry class while the model supplies a supported subtype.
    // The reverse is intentionally false: a generic model answer must not receive credit for missing a human subtype.
    return expectedPatternKey === 'valid_entry' && VALID_ENTRY_SUBTYPES.has(predictedPatternKey);
}
...
export function derivePersonalPatterns(examples = [], { minSample = 8 } = {}) {
    ...
    return [...groups.entries()].filter(([, rows]) => rows.length >= minSample).map(([patternKey, rows]) => {
        ...
        const comparisonOutcomes = withOutcome.filter((item) => !rowSet.has(item)).map((item) => numberOrNull(item.outcome.pnl));
        const wins = outcomes.filter((value) => value > 0).length;
        const losses = outcomes.filter((value) => value < 0).length;
        const baselineWins = comparisonOutcomes.filter((value) => value > 0).length;
        const rate = outcomes.length ? wins / outcomes.length : null;
        const baselineRate = comparisonOutcomes.length ? baselineWins / comparisonOutcomes.length : null;
        const interval = wilsonInterval(wins, outcomes.length);
        const baselineInterval = wilsonInterval(baselineWins, comparisonOutcomes.length);
        const interval90 = wilsonInterval(wins, outcomes.length, 1.645);
        const baselineInterval90 = wilsonInterval(baselineWins, comparisonOutcomes.length, 1.645);
        const separated95 = rate != null && baselineRate != null
            && (interval[0] > baselineInterval[1] || interval[1] < baselineInterval[0]);
        const separated90 = rate != null && baselineRate != null
            && (interval90[0] > baselineInterval90[1] || interval90[1] < baselineInterval90[0]);
        const reliability = outcomes.length >= 30 && comparisonOutcomes.length >= 30 && separated95
            ? 'strong' : outcomes.length >= 15 && comparisonOutcomes.length >= 15 && separated90 ? 'moderate' : 'exploratory';
        return {
            ...
            reliability, interval, baselineInterval, comparisonSampleSize: comparisonOutcomes.length,
            liftInterval95: rate == null || baselineRate == null ? [null, null] : [interval[0] - baselineInterval[1], interval[1] - baselineInterval[0]],
        };
...
async function currentMemory(candidate, limit = 5, { cutoffTradeDate = null, excludeExampleId = null } = {}) {
    ...
    const rows = await supabaseRest(`ai_learning_examples?user_id=eq.${candidate.user_id}&screenshot_path=not.is.null&reviewed_by=not.is.null&review_status=in.(approved,corrected)&review_note=not.like.%5Bauto%5D%25${cutoffFilter}${excludeFilter}&select=reviewed_pattern_key,ai_pattern_key,source_snapshot,outcome,review_note,visual_evidence,screenshot_path&order=reviewed_at.desc&limit=${Math.min(20, limit + 5)}`);
    return (rows || []).filter((item) =>
        !MEMORY_EXCLUDED_PATTERNS.has(item.reviewed_pattern_key || item.ai_pattern_key),
    ).slice(0, limit).map(({ outcome: _outcome, ...item }) => item);
}

export async function analyzePaperCandidate({ userId, screenshotPath, snapshot = {} } = {}) {
    if (!userId || !screenshotPath) throw new Error('Paper analysis requires a user and screenshot');
    const candidate = {
        user_id: userId,
        trade_date: new Date().toISOString().slice(0, 10),
        trade_key: `paper:${userId}:${Date.now()}`,
        screenshot_path: screenshotPath,
        source_snapshot: {
            ticker: compact(snapshot.ticker, 20),
            direction: compact(snapshot.direction, 10),
            entryPrice: numberOrNull(snapshot.entryPrice),
            criteria: compact(snapshot.criteria, 300),
            exceptions: compact(snapshot.exceptions, 300),
            setup: compact(snapshot.setup, 120),
            tradeType: compact(snapshot.tradeType, 120),
            screenshotSet: [{ path: screenshotPath, role: 'pre_entry' }],
        },
        outcome: { hidden: true },
    };
    const preliminary = await geminiGenerate(candidate, [], { blind: true });
    if (!isMemoryEligible(preliminary)) return preliminary;
    ...
    return shouldRunMemoryVerification(preliminary, memory)
        ? geminiGenerate(candidate, [preliminary, ...memory], { blind: true })
        : preliminary;
}

export function guardReplayDecision(analysis = {}, replay = {}) {
    if (replay.decision !== 'ENTER') return replay;
    const evidence = analysis.analysis_features?.evidence || {};
    const visible = Array.isArray(evidence.visible) ? evidence.visible : [];
    const missing = Array.isArray(evidence.missing) ? evidence.missing : [];
    const scores = analysis.analysis_features?.processScores || {};
    const scoreReady = ['setupValidity', 'contextFit', 'entryQuality']
        .every((key) => Number.isFinite(Number(scores[key])) && Number(scores[key]) >= 65);
    const criticalMissing = missing.some((item) => /trigger|confirmation|entry marker|execution|direction/i.test(String(item)));
    if (Number(analysis.ai_confidence || 0) >= 0.7 && visible.length >= 2 && scoreReady && !criticalMissing) return replay;
    return {
        ...replay,
        decision: 'WAIT',
        reason: `Guarded WAIT: entry evidence is not ready. ${replay.reason || ''}`.trim(),
        guardReasons: [
            ...(Number(analysis.ai_confidence || 0) < 0.7 ? ['confidence_below_70'] : []),
            ...(visible.length < 2 ? ['visible_evidence_below_2'] : []),
            ...(!scoreReady ? ['entry_scores_incomplete'] : []),
            ...(criticalMissing ? ['critical_trigger_missing'] : []),
        ],
    };
}

export async function runMarketReplay(candidate) {
    const stages = [{ stage: 'context', cropRatio: 0.58 }, { stage: 'confirmation', cropRatio: 0.78 }];
    const sequence = [];
    let finalAnalysis = null;
    for (const stage of stages) {
        const stagedCandidate = {
            ...candidate,
            source_snapshot: { ...candidate.source_snapshot, marketReplay: stage },
        };
        console.info(`[AI market replay] ${candidate.source_snapshot?.ticker || 'screenshot'} · ${stage.stage} · ${Math.round(stage.cropRatio * 100)}% visible`);
        finalAnalysis = await geminiGenerate(stagedCandidate, [], { blind: true });
        const replay = guardReplayDecision(finalAnalysis, finalAnalysis.analysis_features?.replay || { decision: 'WAIT' });
        sequence.push({ ...stage, ...replay, pattern: finalAnalysis.ai_pattern_key, confidence: finalAnalysis.ai_confidence });
        if (replay.decision === 'ENTER' || replay.decision === 'SKIP') break;
    }
    finalAnalysis.analysis_features.marketReplay = {
        outcomeBlind: true,
        sequence,
        finalDecision: sequence.at(-1)?.decision || 'WAIT',
        revealedOutcomeAfterDecision: outcomeGroup(candidate),
    };
...
export function selectFreshCandidateBatch(fresh = [], batchSize = MAX_BATCH, candidateOffset = 0) {
    ...
    const limit = Math.max(1, Math.min(MAX_BATCH, batchSize));
    const rotated = [...fresh.slice(safeOffset), ...fresh.slice(0, safeOffset)];
    const vision = rotated.filter((item) => item.screenshot_path).slice(0, Math.min(MAX_VISION_PER_BATCH, limit));
    const journalOnly = rotated.filter((item) => !item.screenshot_path).slice(0, Math.max(0, limit - vision.length));
    return [...vision, ...journalOnly];
}
    ...
    try {
        ...
        for (const [index, candidate] of batch.entries()) {
            ...
            try {
                ...
                const replayEnabled = String(version).startsWith('market-replay-v5') && Boolean(candidate.screenshot_path);
                const preliminary = replayEnabled
                    ? await runMarketReplay(candidate)
                    : await geminiGenerate(candidate, [], { blind: true });
                let memory = [];
                let analysis = preliminary;
                if (!replayEnabled && isMemoryEligible(preliminary)) {
                    const memoryCandidate = {
                        ...candidate,
                        source_snapshot: { ...candidate.source_snapshot, aiFeatures: preliminary.analysis_features },
                        analysis_features: preliminary.analysis_features,
                    ...
                    memory = await currentMemory(memoryCandidate, 8);
                    if (shouldRunMemoryVerification(preliminary, memory)) {
                        console.info(`[AI learning] ${progress} Comparing with ${memory.length} remembered examples`);
                        analysis = await geminiGenerate(candidate, [preliminary, ...memory], { blind: true });
                    } else {
                        console.info(`[AI learning] ${progress} No relevant reviewed memory; keeping independent pass`);
                    }
                } else {
                    console.info(`[AI learning] ${progress} Low-evidence result; skipping redundant memory pass`);
                }
                const { analysis_features: analysisFeatures, ...analysisColumns } = analysis;
                analysisFeatures.verification = {
                    mode: replayEnabled ? 'outcome_blind_market_replay' : (analysis === preliminary ? 'independent_single_pass' : 'reviewed_memory_second_pass'),
...
export async function processNextLearningJob({ userId = null } = {}) {
    ...
    try {
        ...
        const versionExamples = await supabaseRest(`ai_learning_examples?prompt_version=eq.${encodeURIComponent(job.prompt_version)}&select=id&limit=1000`);
        const versionProcessed = (versionExamples || []).length;
        const madeProgress = Number(run.processed_count || 0) > 0 || versionProcessed > Number(job.processed_count || 0);
        ...
        const noProgress = !madeProgress && Number(run.remaining_count || 0) > 0;
        ...
        const patch = {
            status,
            processed_count: versionProcessed,
        ...
        if (status === 'completed') {
            try {
                evaluation = await runEvaluationSuite({ version: job.prompt_version });
                const versions = await supabaseRest(`ai_learning_versions?active=eq.true&prompt_version=eq.${encodeURIComponent(job.prompt_version)}&select=id,metrics&limit=1`);
                if (versions?.[0]?.id) {
                    const compactEvaluation = { ...evaluation, results: undefined, evaluatedAt: new Date().toISOString() };
                    await supabaseRest(`ai_learning_versions?id=eq.${versions[0].id}`, {
                        method: 'PATCH', body: JSON.stringify({ metrics: { ...(versions[0].metrics || {}), evaluation: compactEvaluation } }),
                    });
                }
            } catch (evaluationError) {
                console.warn('[AI evaluation] Automatic post-training evaluation failed', evaluationError?.message || evaluationError);
            }
        }
        return { job: updated?.[0] || { ...job, ...patch }, run, evaluation };
    } catch (error) {
        const now = new Date().toISOString();
...
export function summarizeEvaluationResults(results = [], { totalGoldCases = results.length, minimumGoldCases = 30, minimumTestCases = 5 } = {}) {
    ...
    const semanticCorrect = results.filter((item) => item.compatibleMatch ?? item.exactMatch).length;
    ...
    const goldReady = totalGoldCases >= minimumGoldCases;
    const holdoutReady = total >= minimumTestCases;
    return {
        total,
        totalGoldCases, minimumGoldCases, minimumTestCases, goldReady, holdoutReady,
        qualityStatus: goldReady && holdoutReady ? 'measured' : !goldReady ? 'insufficient_gold' : 'insufficient_holdout',
        exactAccuracy: total ? correct / total : null,
        semanticAccuracy: total ? semanticCorrect / total : null,
        selectiveAccuracy: answered.length ? answered.filter((item) => item.exactMatch).length / answered.length : null,
        selectiveSemanticAccuracy: answered.length ? answered.filter((item) => item.compatibleMatch ?? item.exactMatch).length / answered.length : null,
...
export async function runEvaluationSuite({ version = PROMPT_VERSION, limit = 100 } = {}) {
    const [cases, activeCases] = await Promise.all([
        supabaseRest(`ai_evaluation_cases?active=eq.true&dataset_split=eq.test&select=*&order=trade_date.asc,created_at.asc&limit=${Math.max(1, Math.min(500, limit))}`),
        supabaseRest('ai_evaluation_cases?active=eq.true&select=id&limit=500'),
    ]);
    const results = [];
    for (const evaluationCase of cases || []) {
        ...
        try {
            ...
            result = { analysis: null, metrics: { exactMatch: false, compatibleMatch: false, evidenceComplete: false, abstained: true } };
        }
    ...
    return { ...summarizeEvaluationResults(results, { totalGoldCases: activeCases?.length || 0 }), results };
}
