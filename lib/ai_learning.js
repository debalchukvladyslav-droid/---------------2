...
export function buildCandidates(rows = [], profileContexts = new Map(), hashVersion = PROMPT_VERSION) {
    ...
    for (const row of rows) {
        ...
        trades.forEach((trade, index) => {
            ...
            const contentHash = crypto.createHash('sha256').update(JSON.stringify({ promptVersion: hashVersion, tradeKey, snapshot, outcome, screenshot })).digest('hex');
            output.push({
                user_id: row.user_id,
...
export async function runLearningBatch({ triggerType = 'manual', userId = null, batchSize = MAX_BATCH, forceReanalyze = false, version = PROMPT_VERSION } = {}) {
    const runRows = await supabaseRest('ai_learning_runs?select=*', {
        method: 'POST', headers: { Prefer: 'return=representation' },
        body: JSON.stringify({ trigger_type: triggerType, created_by: userId, model_name: AI_MODEL, prompt_version: version }),
    });
    const run = runRows[0];
    const errors = [];
    ...
    try {
        ...
        const candidates = buildCandidates(rows || [], profileContexts, version);
        ...
        const fresh = forceReanalyze ? candidates : candidates.filter((candidate) => !existing.has(candidate.content_hash));
