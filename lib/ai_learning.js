...
export const PROMPT_VERSION = 'entry-movement-v2';
...
const MAX_BATCH = 8;
const AUTO_MEMORY_CONFIDENCE = 0.55;
...
async function promotePendingExamples(limit = 30) {
    ...
    for (const example of pending || []) {
        const canEnterMemory = Number(example.ai_confidence) >= AUTO_MEMORY_CONFIDENCE
            && !['unclear', 'insufficient_data'].includes(example.ai_pattern_key);
        const patch = {
    ...
    try {
        ...
        for (const candidate of fresh.slice(0, Math.max(1, Math.min(20, batchSize)))) {
            try {
                ...
                const canEnterMemory = analysis.ai_confidence >= AUTO_MEMORY_CONFIDENCE
                    && !['unclear', 'insufficient_data'].includes(analysis.ai_pattern_key);
