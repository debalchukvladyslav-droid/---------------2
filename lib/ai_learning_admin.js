...
async function queuePayload(query = {}) {
    ...
    const status = ['pending', 'approved', 'corrected', 'rejected'].includes(String(query.status)) ? String(query.status) : 'approved';
...
async function overviewPayload() {
    const [examples, runs, versions, journalRows, profiles] = await Promise.all([
        supabaseRest('ai_learning_examples?is_current=eq.true&select=user_id,review_status,review_note,ai_pattern_key,reviewed_pattern_key,ai_confidence,screenshot_path,source_snapshot,created_at,reviewed_at'),
    ...
    const adminReviewed = reviewed.filter((row) => !String(row.review_note || '').startsWith('[auto]'));
    const agreed = adminReviewed.filter((row) => row.review_status === 'approved').length;
    const byPattern = {};
    ...
    return {
        ...
        qualityHistory: [...reviewed].sort((a, b) => String(a.reviewed_at).localeCompare(String(b.reviewed_at))).map((row, index, all) => {
            ...
            return { date: row.reviewed_at, accuracy: slice.filter((item) => Number(item.ai_confidence) >= 0.55).length / slice.length };
        }),
    };
}
...
export default async function aiLearningAdminHandler(req, res) {
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (!['run', 'new-training'].includes(body.action)) return sendJson(res, 400, { ok: false, error: 'Unknown action' });
            ...
            if (body.action === 'new-training') {
                ...
                version = `entry-memory-v3.${memoryVersion}`;
                if (active?.id) {
