...
import { assignChronologicalSplits, createEmbedding, derivePersonalPatterns, embeddingText, PATTERN_KEYS, processNextLearningJob, runEvaluationSuite, runLearningBatch } from './ai_learning.js';
...
async function overviewPayload() {
    const [examples, runs, versions, journalRows, profiles, personalPatterns, jobs, evaluationCases] = await Promise.all([
        supabaseRest('ai_learning_examples?is_current=eq.true&select=user_id,review_status,review_note,reviewed_by,ai_pattern_key,reviewed_pattern_key,ai_confidence,screenshot_path,source_snapshot,created_at,reviewed_at'),
        ...
        supabaseRest('ai_evaluation_cases?active=eq.true&select=dataset_split&limit=1000'),
    ]);
    const rows = examples || [];
...
async function syncGoldCases(userId) {
    const reviewed = await supabaseRest('ai_learning_examples?is_current=eq.true&review_status=in.(approved,corrected)&reviewed_by=not.is.null&reviewed_pattern_key=not.is.null&select=id,user_id,reviewed_pattern_key,source_snapshot,trade_date,review_note,reviewed_by&limit=1000');
    const eligible = (reviewed || []).filter((example) =>
        !String(example.review_note || '').startsWith('[auto]'),
    );
    if (eligible.length) {
        ...
        for (const reviewedUserId of new Set(eligible.map((example) => example.user_id))) {
...
async function refreshPersonalPatterns(userId) {
    const examples = await supabaseRest(`ai_learning_examples?user_id=eq.${encodeURIComponent(userId)}&is_current=eq.true&reviewed_by=not.is.null&review_status=in.(approved,corrected)&select=ai_pattern_key,reviewed_pattern_key,review_status,reviewed_by,outcome`);
    ...
    for (const pattern of patterns) {
        await supabaseRest('ai_user_patterns?on_conflict=user_id,dimension,pattern_key', {
            ...
            body: JSON.stringify({
                ...
                statistics: { interval: pattern.interval, baselineInterval: pattern.baselineInterval },
