...
async function currentTrainingCampaign() {
    const versions = await supabaseRest('ai_learning_versions?active=eq.true&select=metrics&limit=1');
    return versions?.[0]?.metrics?.trainingCampaign || null;
}
            ...
            if (body.action === 'run') {
                ...
                if (queued.job) {
                    return sendJson(res, 200, {
                        ok: true, ...queued,
                        campaign: queued.run ? await updateCampaignAfterRun(queued.run) : await currentTrainingCampaign(),
                    });
                }
                const activeJobs = await supabaseRest('ai_learning_jobs?status=in.(running,processing)&select=*&order=started_at.asc&limit=1');
                if (activeJobs?.[0]) {
                    return sendJson(res, 200, { ok: true, job: activeJobs[0], run: null, campaign: await currentTrainingCampaign() });
                }
            }
