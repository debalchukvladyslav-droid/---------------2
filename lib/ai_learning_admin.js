...
import { analyzePaperCandidate, assignChronologicalSplits, candidateIdentity, createEmbedding, derivePersonalPatterns, embeddingText, PATTERN_KEYS, processNextLearningJob, runEvaluationSuite, runLearningBatch } from './ai_learning.js';
import { diversifyReviewExamples } from './ai_review_priority.js';
import { buildPaperTradeDecision, canEnableLiveExecution, summarizePaperPerformance, validatePaperTradePrices } from './ai_trade_decision.js';
...
async function overviewPayload() {
    const versions = await supabaseRest('ai_learning_versions?active=eq.true&select=*&limit=1');
    const versionFilter = versions?.[0]?.prompt_version ? `&prompt_version=eq.${encodeURIComponent(versions[0].prompt_version)}` : '';
    const [examples, runs, journalRows, profiles, personalPatterns, jobs, evaluationCases, goldExamples, paperSignals] = await Promise.all([
        supabaseRest(`ai_learning_examples?is_current=eq.true${versionFilter}&select=id,user_id,journal_day_id,trade_date,trade_key,review_status,review_note,reviewed_by,ai_pattern_key,reviewed_pattern_key,ai_confidence,screenshot_path,source_snapshot,outcome,created_at,reviewed_at`),
        ...
        supabaseRest('ai_paper_signals?select=action,outcome_r,observed_at,resolved_at&order=observed_at.asc&limit=2000'),
    ]);
    const rows = examples || [];
    ...
    const visualGold = (goldExamples || []).filter((row) => activeGoldIds.has(row.id) && !String(row.review_note || '').startsWith('[auto]'));
    ...
    const byPattern = {};
    rows.filter((row) => row.screenshot_path).forEach((row) => {
        const key = row.ai_pattern_key || 'unclear';
        const bucket = byPattern[key] || { key, total: 0, agreed: 0 };
        bucket.total++;
        if (row.reviewed_pattern_key === row.ai_pattern_key && row.reviewed_by && !String(row.review_note || '').startsWith('[auto]')) bucket.agreed++;
    ...
    const paper = summarizePaperPerformance(paperSignals || []);
    const paperTrading = {
        ...paper,
        goldCount: (evaluationCases || []).length,
        holdoutCount: (evaluationCases || []).filter((row) => row.dataset_split === 'test').length,
        selectiveAccuracy: Number(versions?.[0]?.metrics?.evaluation?.selectiveAccuracy || 0),
        coverage: Number(versions?.[0]?.metrics?.evaluation?.coverage || 0),
    };
    paperTrading.liveExecutionReady = canEnableLiveExecution({
        ...paperTrading,
        maxPaperDrawdownR: paperTrading.maxDrawdownR,
    });
    return {
        summary: {
            candidateTrades,
            processed: rows.length,
            screenshotBacked: rows.filter((row) => row.screenshot_path).length,
            visionClassified: rows.filter((row) => row.screenshot_path && row.ai_pattern_key !== 'insufficient_data').length,
            visionInsufficient: rows.filter((row) => row.screenshot_path && row.ai_pattern_key === 'insufficient_data').length,
            journalOnly: rows.filter((row) => !row.screenshot_path).length,
        ...
        runs: runs || [],
        patterns: Object.values(byPattern).map((item) => ({ ...item, accuracy: item.agreed ? item.agreed / item.total : null })).sort((a, b) => b.total - a.total),
        ...
        paperTrading,
        paperScreenshots: [...new Map(rows.filter((row) => row.screenshot_path).map((row) => [row.screenshot_path, {
            path: row.screenshot_path,
            ticker: row.source_snapshot?.ticker || '',
...
async function currentTrainingCampaign({ resumeIdleForJob = false } = {}) {
    const versions = await supabaseRest('ai_learning_versions?active=eq.true&select=id,metrics&limit=1');
    const version = versions?.[0];
    const campaign = version?.metrics?.trainingCampaign || null;
    if (!version || !campaign || !resumeIdleForJob || campaign.status !== 'idle' || Date.parse(campaign.endsAt || 0) <= Date.now()) return campaign;
    const resumed = { ...campaign, status: 'running', completedAt: null, resumedAt: new Date().toISOString() };
    await supabaseRest(`ai_learning_versions?id=eq.${version.id}`, {
        method: 'PATCH', body: JSON.stringify({ metrics: { ...(version.metrics || {}), trainingCampaign: resumed } }),
    });
    return resumed;
}
...
async function updateCampaignAfterRun(run, { job = null } = {}) {
    const versions = await supabaseRest('ai_learning_versions?active=eq.true&select=id,metrics&limit=1');
    const version = versions?.[0];
    const campaign = version?.metrics?.trainingCampaign;
    const durableWorkRemaining = job?.status === 'running' && Number(job?.remaining_count || 0) > 0;
    const campaignWindowOpen = Date.parse(campaign?.endsAt || 0) > Date.now();
    const resumableIdle = campaign?.status === 'idle' && durableWorkRemaining && campaignWindowOpen;
    if (!version || (campaign?.status !== 'running' && !resumableIdle)) return campaign || null;
    const expired = !campaignWindowOpen;
    const noWork = !durableWorkRemaining && Number(run?.processed_count || 0) === 0 && Number(run?.failed_count || 0) === 0;
    const next = {
        ...campaign,
        status: expired ? 'completed' : noWork ? 'idle' : 'running',
        batches: job ? Number(job.batch_count || 0) : Number(campaign.batches || 0) + 1,
        processed: job ? Number(job.processed_count || 0) : Number(campaign.processed || 0) + Number(run?.processed_count || 0),
        failures: job ? Number(job.failed_count || 0) : Number(campaign.failures || 0) + Number(run?.failed_count || 0),
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (body.action === 'evaluate') {
                ...
                const versions = await supabaseRest('ai_learning_versions?active=eq.true&select=id,prompt_version,metrics&limit=1');
                const evaluation = await runEvaluationSuite({ version: versions?.[0]?.prompt_version });
                if (versions?.[0]?.id) {
                    await supabaseRest(`ai_learning_versions?id=eq.${versions[0].id}`, {
                        method: 'PATCH',
                        body: JSON.stringify({ metrics: { ...(versions[0].metrics || {}), evaluation: { ...evaluation, results: undefined, evaluatedAt: new Date().toISOString() } } }),
                    });
                }
                return sendJson(res, 200, { ok: true, gold, evaluation });
            }
            if (body.action === 'paper-decision') {
                const snapshot = body.snapshot && typeof body.snapshot === 'object' ? body.snapshot : {};
                const screenshotPath = String(body.screenshotPath || '').trim().slice(0, 500);
                if (!screenshotPath || !snapshot.ticker) return sendJson(res, 400, { ok: false, error: 'Ticker and pre-entry screenshot are required' });
                const analysis = await analyzePaperCandidate({ userId: user.id, screenshotPath, snapshot });
                const patternRows = await supabaseRest(`ai_user_patterns?user_id=eq.${encodeURIComponent(user.id)}&pattern_key=eq.${encodeURIComponent(analysis.ai_pattern_key)}&active=eq.true&select=sample_size,reliability,win_rate&limit=1`);
                let decision = buildPaperTradeDecision(analysis, snapshot, patternRows?.[0] || null);
                const validPrices = validatePaperTradePrices(decision.action, snapshot.entryPrice, snapshot.stopPrice, snapshot.targetPrice);
                if (decision.action !== 'SKIP' && !validPrices) {
                    decision = { ...decision, action: 'SKIP', reasons: [...decision.reasons, 'valid_entry_stop_target_required'], risk: { ...decision.risk, maxRiskR: 0 } };
                }
                const now = new Date().toISOString();
                const inserted = await supabaseRest('ai_paper_signals?select=*', {
                    method: 'POST', headers: { Prefer: 'return=representation' },
                    body: JSON.stringify({
                        user_id: user.id, observed_at: now, source_cutoff_at: now,
                        ticker: String(snapshot.ticker).trim().toUpperCase().slice(0, 20),
                        action: decision.action, pattern_key: decision.pattern,
                        confidence: decision.confidence, decision,
                        entry_price: decision.action === 'SKIP' ? null : Number(snapshot.entryPrice),
                        stop_price: decision.action === 'SKIP' ? null : Number(snapshot.stopPrice),
                        target_price: decision.action === 'SKIP' ? null : Number(snapshot.targetPrice),
                    }),
                });
                return sendJson(res, 200, { ok: true, analysis, decision, signal: inserted?.[0] || null });
            }
            ...
            if (body.action === 'run') {
                const queued = await processNextLearningJob({ userId: user.id });
                if (queued.job) {
                    return sendJson(res, 200, {
                        ok: true, ...queued,
                        campaign: queued.run ? await updateCampaignAfterRun(queued.run, { job: queued.job }) : await currentTrainingCampaign(),
                    });
                }
                const activeJobs = await supabaseRest('ai_learning_jobs?status=in.(running,processing)&select=*&order=started_at.asc&limit=1');
                if (activeJobs?.[0]) {
                    return sendJson(res, 200, { ok: true, job: activeJobs[0], run: null, campaign: await currentTrainingCampaign({ resumeIdleForJob: true }) });
                }
            }
            ...
            if (body.action === 'new-training') {
                ...
                version = `market-replay-v5.${memoryVersion}`;
                if (active?.id) {
