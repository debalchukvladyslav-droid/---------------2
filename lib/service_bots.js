...
export async function authenticateServiceBot(req) {
    ...
    const rows = await supabaseRest(
        `bots?api_key_hash=eq.${encodeURIComponent(hashServiceBotApiKey(apiKey))}&select=*&limit=1`,
    );
    const bot = rows?.[0];
