...
export async function buildServiceBotResponse(req, endpoint) {
    ...
    const cacheKey = JSON.stringify({ endpoint, botId: bot.id, userId: bot.user_id || 'all_traders', range, limit, topLimit });
    ...
    const sections = buildServiceBotSnapshot(rows || [], range, { limit, top_limit: topLimit, profilesById });
    const baseMeta = {
        subject: {
...
export function cleanBotPayload(body = {}) {
    ...
    return { name: name || 'Service bot', userId: allTraders ? '' : userId, apiKey, allTraders, enabled };
}
