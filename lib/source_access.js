...
export function profileOwnsTraderSheet(profile, sheetTitle) {
    const owner = normalizeSheetOwnerTitle(profile?.last_name);
    const title = normalizeSheetOwnerTitle(sheetTitle);
    return Boolean(owner) && owner === title;
}
...
export function visibleSpreadsheetSheets(sheets, connection) {
    ...
    if (connection?.bootstrap) {
        return list.filter((sheet) => profileOwnsTraderSheet({ last_name: connection.ownerSheetTitle }, sheet.title));
    }
...
export function assertBootstrapSheetRange(connection, range, explicitTitle = '') {
    ...
    if (!profileOwnsTraderSheet({ last_name: connection.ownerSheetTitle }, requested)) throw sourceAccessError();
    return inRange || requested;
}

export async function sourceProfile(userId) {
    return (await supabaseRest(`profiles?id=eq.${encodeURIComponent(userId)}&select=id,role,last_name&limit=1`))?.[0] || {};
}
