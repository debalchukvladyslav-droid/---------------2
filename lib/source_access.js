...
export function shouldProvisionDriveConnection(error, { folderId = '', write = false, connectionId = '' } = {}) {
    return error?.code === 'SOURCE_CONNECTION_REQUIRED'
        && !write
        && !connectionId
        && DRIVE_FOLDER_ID.test(String(folderId || ''));
}
...
export function canBootstrapSharedTraderSheet(profile, spreadsheetId, sheetTitle = '') {
    ...
    if (!/^[A-Za-z0-9_-]{20,}$/.test(String(spreadsheetId || ''))) return false;
    if (!normalizeSheetOwnerTitle(profile.last_name)) return false;
    if (sheetTitle && !cleanSheetOwnerText(sheetTitle)) return false;
    return true;
}
...
export function visibleSpreadsheetSheets(sheets, connection) {
    ...
    if (connection?.bootstrap) return list;
...
export function assertBootstrapSheetRange(connection, range, explicitTitle = '') {
    ...
    if (!cleanSheetOwnerText(requested)) throw sourceAccessError('Оберіть аркуш у таблиці.');
    return requested;
}
