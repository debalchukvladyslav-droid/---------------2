    ...
    async getLiveQuotes(symbols) {
        ...
        try {
            ...
            symbols.forEach((symbol, index) => {
                ...
                const premarket = state === 'PRE' || state === 'PREPRE';
                const previous = Number(row.regularMarketPreviousClose);
                const last = Number(row.regularMarketPrice);
                const open = Number(row.regularMarketOpen);
                const pre = Number(row.preMarketPrice);
                const changePct = Number(row.regularMarketChangePercent);
                const ratio = (price, base) => (price > 0 && base > 0 ? price / base - 1 : null);
                const fromPrevClose = premarket
                    ? ratio(pre, previous)
                    : (Number.isFinite(changePct) ? changePct / 100 : ratio(last, previous));
                const fromOpen = premarket ? null : ratio(last, open);
                let changePoints = Number.isFinite(last) && Number.isFinite(previous) ? last - previous : null;
                if (premarket && Number.isFinite(pre) && Number.isFinite(previous)) changePoints = pre - previous;
                if (mapped[index] === '^TNX' && changePoints != null) changePoints *= 10;
                quotes[symbol] = {
                    change: fromPrevClose,
                    fromPrevClose,
                    fromOpen,
                    changePoints,
                };
        ...
        return {
            quotes,
            incomplete: symbols.some((symbol) => quotes[symbol]?.fromPrevClose == null && quotes[symbol]?.fromOpen == null),
        };
    }
}
