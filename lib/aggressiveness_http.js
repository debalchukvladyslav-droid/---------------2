...
import { buildLiveNextSession, buildNextSessionResearch, loadMechanicalSignals } from '../lib/next_session_service.js';
...
const SHARED_RESPONSE_MS = 45000;

function shareResponse(key, load) {
    const now = Date.now();
    const current = sharedResponses.get(key);
    if (current?.value && current.expires > now) return Promise.resolve(current.value);
    if (current?.task) return current.task;
    const task = Promise.resolve().then(load).then((value) => {
        const reusable = value && value.status !== 'incomplete' && value.incomplete !== true;
        sharedResponses.set(key, reusable ? { value, expires: Date.now() + SHARED_RESPONSE_MS } : {});
        return value;
    }).catch((error) => {
        sharedResponses.delete(key);
        throw error;
    });
    sharedResponses.set(key, { task });
    return task;
}
...
export default async function handler(req, res) {
    ...
    if (auth.authDegraded) return json(res, 200, degradedMarketPayload());
    ...
    try {
        ...
        if (view === 'next') {
            const payload = await shareResponse('next', () => buildLiveNextSession(process.env, fetch, new Date(), {
                store: safeStore(),
                provider: safeProvider(auth),
            }));
            return json(res, 200, payload);
        }
        if (view === 'next-backtest') {
            const admin = await isAdmin(auth);
            if (!admin.ok) return json(res, admin.status, { message: admin.message });
            const report = await runNextBacktest(auth);
            return json(res, 200, { ok: true, ...report });
        }
        const payload = await shareResponse('gauge', () => buildGauge({
            now: new Date(),
            env: process.env,
            store: safeStore(),
            provider: safeProvider(auth),
        ...
        return json(res, 200, degradedMarketPayload());
    }
}

export function degradedMarketPayload(now = new Date()) {
    return {
        status: 'incomplete',
        incomplete: true,
        ...
        sessionDate: displaySessionDate(now),
    };
}

async function runNextBacktest(auth) {
    const provider = safeProvider(auth);
    if (!provider) return { ok: false, message: 'Data incomplete', days: 0, buckets: [], lags: [] };
    const today = displaySessionDate(new Date());
    const history = await loadScaledHistory(provider, safeStore(), today, process.env, {
        from: addCalendarDays(today, -520),
        to: previousTradingDay(today),
    });
    const signals = await loadMechanicalSignals();
    return buildNextSessionResearch({ barsByKey: history.barsByKey, signals });
}
...
async function verifyUser(req) {
    ...
    try {
        ...
        return { ok: true, token, url, anon, authDegraded: true };
    }
}
...
const MARKET_API_PAUSE_MS = 15 * 60 * 1000;
let marketApiBlockedUntil = 0;

function marketApiBlocked() {
    return Date.now() < marketApiBlockedUntil;
}

function noteMarketApiFailure(error) {
    ...
    if (status === 404 || status === 401 || status === 403 || status >= 500
        ...
        marketApiBlockedUntil = Date.now() + MARKET_API_PAUSE_MS;
    }
}

async function marketRest(path, options = {}) {
    if (marketApiBlocked()) {
        const error = new Error('Market cache skipped after a Data API failure');
        error.status = 503;
        throw error;
    }
    try {
        return await supabaseRest(path, { attempts: 1, ...options });
    } catch (error) {
        noteMarketApiFailure(error);
        throw error;
    }
}
...
function createStore() {
    return {
        async readBars(symbol, from, to) {
            const rows = await marketRest(`daily_market_bars?select=bar_date,open,high,low,close&symbol=eq.${encodeURIComponent(symbol)}&bar_date=gte.${from}&bar_date=lte.${to}&order=bar_date.asc`);
            return (rows || []).map(toBar);
        },
        async writeBars(symbol, bars) {
            if (marketApiBlocked()) return;
            ...
            for (let index = 0; index < rows.length; index += 400) {
                await marketRest('daily_market_bars?on_conflict=symbol,bar_date', {
                    method: 'POST',
        ...
        async readRegime(date, version) {
            const rows = await marketRest(`daily_market_regime?select=*&date=eq.${date}&score_version=eq.${version}&limit=1`);
            return rows?.[0] || null;
        },
        async readLatest(version) {
            const rows = await marketRest(`daily_market_regime?select=*&score_version=eq.${version}&order=date.desc&limit=1`);
            return rows?.[0] || null;
        },
        async readPrevious(date, version) {
            const rows = await marketRest(`daily_market_regime?select=live_score,date&score_version=eq.${version}&date=lt.${date}&order=date.desc&limit=1`);
            return rows?.[0] || null;
        },
        async insertRegime(row) {
            await marketRest('daily_market_regime?on_conflict=date,score_version', {
                method: 'POST',
        ...
        async insertRegimes(rows) {
            if (!rows?.length || marketApiBlocked()) return false;
        ...
        async patchRegime(date, version, patch) {
            await marketRest(`daily_market_regime?date=eq.${date}&score_version=eq.${version}&live_frozen=eq.false`, {
                method: 'PATCH',
        ...
        async readMechanical(from, to) {
            const rows = await marketRest(`daily_market_regime?select=date,mechanical_entries,mechanical_total_r,mechanical_win_rate,mechanical_avg_winner,mechanical_avg_loser&score_version=eq.${SCORE_VERSION}&date=gte.${from}&date=lte.${to}&mechanical_entries=not.is.null`);
            const byDate = {};
