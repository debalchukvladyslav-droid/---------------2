...
function reviewDimension(example, name) {
    ...
    if (name === 'outcome') {
        const pnl = Number(example.outcome?.pnl);
        return !Number.isFinite(pnl) || pnl === 0 ? 'unknown' : pnl > 0 ? 'profit' : 'loss';
    }
...
export function diversifyReviewExamples(examples = []) {
    ...
    const selected = [];
    const counts = { ticker: new Map(), date: new Map(), pattern: new Map(), outcome: new Map() };
    ...
    while (remaining.length) {
        ...
        for (let index = 0; index < remaining.length; index++) {
            ...
            const diversityPenalty = count('ticker', item) * 5
                + count('outcome', item) * 3
                + count('pattern', item) * 2
                + count('date', item);
