...
function patternReliabilityScore(patternStats) {
    const numeric = Number(patternStats?.reliability ?? patternStats?.precision);
    if (Number.isFinite(numeric)) return numeric;
    return ({ exploratory: 0.25, moderate: 0.7, strong: 0.9 })[String(patternStats?.reliability || '')] || 0;
}
...
export function buildPaperTradeDecision(analysis = {}, snapshot = {}, patternStats = null) {
    ...
    const reliability = patternReliabilityScore(patternStats);
...
export function validatePaperTradePrices(action, entryPrice, stopPrice, targetPrice) {
    const entry = Number(entryPrice); const stop = Number(stopPrice); const target = Number(targetPrice);
    if (![entry, stop, target].every((value) => Number.isFinite(value) && value > 0)) return false;
    if (action === 'LONG') return stop < entry && entry < target;
    if (action === 'SHORT') return target < entry && entry < stop;
    return action === 'SKIP';
}
