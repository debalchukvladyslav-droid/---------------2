...
export async function buildGauge(options = {}) {
    ...
    if (needsLive && provider?.getLiveQuotes) {
        ...
        try {
            ...
            quotesReady = ['SPY', 'QQQ', 'IWM'].every((key) => {
                const quote = quoteMap[key];
                const move = phase === 'premarket' ? quote?.fromPrevClose : quote?.fromOpen;
                return Number.isFinite(move);
            });
        } catch {
            quotesReady = false;
        }
    }
    const liveActive = phase === 'premarket' || phase === 'open';
    computed.confidence = scoreConfidence({
        soft: computed.soft,
        percentiles: Object.values(computed.components?.detail || {}),
        microSmall: computed.components?.microSmall,
        speculative: computed.components?.speculative,
        broad: computed.components?.broad,
        stress: computed.components?.stress,
        liveActive,
        liveFresh: !liveActive || quotesReady,
    });
    const raw = quotesReady ? rawLiveAdjustment(quoteMap, { priorShock, phase }) : null;
    ...
    const progression = recentAggressiveness(history.barsByKey, sessionDate, 14);
    ...
    if (existing && decision.action !== 'insert') {
        const live = decision.action === 'patch'
            ? Number(decision.patch.live_adjustment)
            : Number(existing.live_adjustment) || 0;
        const liveScore = finalAggressiveness(
            computed.baseScore,
        ...
        const stored = payloadFromComputed({
            ...computed,
            liveScore,
            liveAdjustment: live,
            ...
            label: status.label,
            tone: status.tone,
            components: { ...computed.components, liveAdjustment: live },
        }, {
            updatedAt,
            persisted,
            liveFeedIncomplete: needsLive && !quotesReady,
        });
        stored.delta = Number.isFinite(previousScore) ? round(status.score - previousScore, 1) : null;
        stored.history = stampProgression(progression, sessionDate, status.score);
    ...
    return {
        ...
        history: stampProgression(progression, sessionDate, status.score),
...
function stampProgression(series, sessionDate, displayScore) {
    const next = (series || []).map((point) => ({ ...point }));
    const last = next.at(-1);
    const score = Number(displayScore);
    if (last && last.date === sessionDate && Number.isFinite(score)) last.score = Math.round(score);
    return next;
}
