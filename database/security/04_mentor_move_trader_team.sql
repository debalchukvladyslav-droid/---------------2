-- Allow mentors to move traders from their own team to another team from the
-- admin panel, without granting full profile administration.
...
BEGIN
    ...
    IF COALESCE(target.team, 'Без куща') <> COALESCE(caller.team, 'Без куща') THEN
        RAISE EXCEPTION 'Mentor can move traders only from own team';
    END IF;
