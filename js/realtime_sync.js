...
import { syncDataNow, wasDayRecentlySaved } from './storage.js';
...
async function refresh(kind, payload = null) {
    if (kind === 'journal') {
        ...
        if (/^\d{4}-\d{2}-\d{2}$/.test(tradeDate) && wasDayRecentlySaved(tradeDate)) return;
        await syncDataNow().catch(() => {});
    } else {
...
function receive(payload, userId) {
    ...
    const key = `${kind}:${row.trade_date || row.id || 'all'}`;
    clearTimeout(timers.get(key));
    timers.set(key, setTimeout(() => {
        timers.delete(key);
        void refresh(kind, payload);
    }, 250));
}
