...
const steps = [
    ...
    { id: 'sheet-mode', tab: 'table', target: '.sheet-import-mode-toggle', title: '1. Оберіть режим таблиці', text: '«Основна» — ваша поточна робоча таблиця, з якої дані регулярно потрапляють у журнал. «Накопичувальна» — додаткове історичне джерело: його рядки зберігаються для контексту, але не повинні дублювати основну таблицю.', group: 'mapping', prepare: 'sheet-open' },
    { id: 'sheet-source', tab: 'table', target: '.sheet-preset-picker', title: '2. Підключіть правильне джерело', text: 'Оберіть кнопку зі своєю групою або вставте власне посилання чи spreadsheet ID. Якщо у файлі кілька листів, після завантаження виберіть потрібний лист у списку. До мапінгу переходьте лише після появи прев’ю клітинок.', group: 'mapping', prepare: 'sheet-open', action: '.sheet-preset-btn, [data-action="sheet-service-load"]', actionLabel: 'Оберіть або підключіть таблицю', optional: true },
    { id: 'sheet-map', tab: 'table', target: '[data-action="sheet-auto-map"]', title: '3. Запустіть точний автомапінг', text: 'Автомапінг перевіряє заголовки перших 20 рядків і шукає точні назви колонок. Він визначає Дату, Ticker, Тип угоди, Профіт, ризики, виключення, коментарі, вихід та інші поля. Широкі слова на кшталт type, risk або comment навмисно не підбираються, щоб не змішати колонки.', group: 'mapping', prepare: 'sheet-open', action: '[data-action="sheet-auto-map"]', actionLabel: 'Натисніть «Автомапінг»', optional: true },
    { id: 'sheet-required', tab: 'table', target: '#sheet-mapping-status', title: '4. Перевірте обов’язкові поля', text: 'Насамперед перевірте «Дата» і «Тікер» — без них імпорт не запускається. «Класифікація» повинна потрапляти в «У чому виключення», а не в «Тип угоди». Тип угоди має відповідати колонці з реальними значеннями шорт, візуально, фіолетова, виключення тощо.', group: 'mapping', prepare: 'sheet-open' },
    { id: 'sheet-start-row', tab: 'table', target: '#sheet-grid-picker-start-row', title: '5. Перевірте стартовий рядок', text: 'Система знаходить колонку Ticker, а нижче заголовка — перше значення, що починається з латинської літери, наприклад AAPL або BRK.B. Рядок цього тікера стає спільним початком даних для всіх колонок. Якщо показаний неправильний номер, виберіть правильну клітинку вручну.', group: 'mapping', prepare: 'sheet-open' },
    { id: 'sheet-manual', tab: 'table', target: '[data-smart-field="date"]', title: '6. Виправте поля вручну за потреби', text: 'Кожен рядок мапінгу має список колонок і перемикач ручного режиму. Виберіть поле, а потім клікніть по клітинці потрібної колонки у прев’ю. Перевірте не лише Дату й Тікер, а також Профіт, Профіт у ризиках, Тип угоди, Виключення, Вихід і коментарі.', group: 'mapping', prepare: 'sheet-open' },
    { id: 'sheet-preview', tab: 'table', target: '.sheet-grid-picker', title: '7. Перегляньте всю Excel-таблицю', text: 'Кнопки − і + змінюють масштаб прев’ю. Використовуйте горизонтальну та вертикальну прокрутку, щоб перевірити далекі колонки й перші рядки даних. Активне поле показане у верхній частині: клік по клітинці призначає її колонку саме цьому полю.', group: 'mapping', prepare: 'sheet-open' },
    { id: 'sheet-save', tab: 'table', target: '#sheet-save-sync-btn', title: '8. Збережіть і синхронізуйте', text: 'Перед натисканням ще раз перевірте Дату, Тікер і стартовий рядок. Кнопка збереже мапінг для вашого акаунта, прочитає рядки таблиці та зіставить їх з угодами брокера за днем і тікером. Після завершення результат можна перевірити у вкладці «Таблиця».', group: 'mapping', prepare: 'sheet-open', action: '#sheet-save-sync-btn', actionLabel: 'Збережіть мапінг і синхронізуйте', optional: true },
    ...
    { id: 'learn', tab: 'learn', target: '.learn-preference-panel', title: 'Навчання', text: 'Напишіть своє побажання — напрямок, який хочете підтягнути. AI поєднає його зі статистикою журналу й підбере відео. Кнопка «Оновити рекомендації» запускає новий пошук.' },
