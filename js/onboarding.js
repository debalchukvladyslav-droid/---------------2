...
const steps = [
    ...
    { id: 'sync', tab: 'dash', target: '#manual-sync-btn', title: 'Оновлення даних', text: 'Ця кнопка оновлює журнал, таблицю і скріншоти. Журнал також синхронізується сам, коли ви щось змінюєте.' },
    { id: 'calendar', tab: 'calendar', target: '.day-cell', title: 'Календар торгових днів', text: 'Клік по дню відкриває форму цього дня. На огляді форма закрита, щоб не перекривати картки.' },
    { id: 'drive-open', tab: 'screens', target: '[data-action="drive-instruction-toggle"]', title: '1. Відкрийте інструкцію Drive', text: 'У налаштуваннях скріншотів натисніть «Показати інструкцію». Там є три кроки зі зображеннями.', group: 'drive', prepare: 'screens-open', action: '[data-action="drive-instruction-toggle"]', actionLabel: 'Показати інструкцію', optional: true },
    { id: 'drive-share', tab: 'screens', target: '.drive-service-credential-row', title: '2. Скопіюйте пошту', text: 'Натисніть кнопку копіювання праворуч від підсвіченої адреси. У Google Drive вставте цю пошту в поле «Додайте користувачів» і виберіть роль «Редактор».', group: 'drive', prepare: 'screens-open', action: '.drive-service-credential-row .technical-email-copy-btn', actionLabel: 'Натисніть кнопку копіювання біля пошти', optional: true },
    { id: 'drive-folder', tab: 'screens', target: '#drive-service-folder-input', title: '3. Вставте посилання на папку', text: 'Скопіюйте адресу папки Google Drive з адресного рядка та вставте її саме в підсвічене поле. Також можна вставити лише ID — частину адреси після «/folders/».', group: 'drive', prepare: 'screens-open', action: '#drive-service-folder-input', event: 'input', actionLabel: 'Вставте посилання або ID у підсвічене поле', optional: true },
    ...
    { id: 'sheet-optional', tab: 'table', target: '[data-smart-field="profit"]', title: '8. Додаткові колонки', text: 'Профіт, тип угоди, ризики, коментарі та інші поля необов’язкові. Заповнюйте або виправляйте тільки ті колонки, які є у вашій таблиці.', group: 'mapping', prepare: 'sheet-edit', highlightField: 'profit' },
    { id: 'sheet-preview', tab: 'table', target: '.sheet-grid-picker', title: '9. Прев’ю таблиці', text: 'Оберіть потрібне поле, а потім клікніть клітинку відповідної колонки. Підсвічений рядок мапінгу показує, для якого поля зараз вибирається колонка.', group: 'mapping', prepare: 'sheet-edit' },
    { id: 'sheet-save', tab: 'table', target: '#sheet-save-sync-btn', title: '10. Збережіть зміни', text: 'Після ручного редагування збережіть мапінг і синхронізуйте угоди. Якщо ручні зміни не потрібні, достатньо кнопки «Автомапінг».', group: 'mapping', prepare: 'sheet-edit', action: '#sheet-save-sync-btn', actionLabel: 'Збережіть мапінг і синхронізуйте', optional: true },
...
async function prepareStep(step) {
    ...
    if (['sheet-open', 'sheet-edit'].includes(step.prepare) && document.getElementById('sheet-google-panel-toggle')?.getAttribute('aria-expanded') !== 'true') {
        document.getElementById('sheet-google-panel-toggle')?.click();
        await new Promise((resolve) => setTimeout(resolve, 180));
    }
    if (step.prepare === 'sheet-edit') {
        const editButton = document.querySelector('[data-action="sheet-mapping-edit"]');
        if (editButton?.getAttribute('aria-expanded') !== 'true') {
            editButton?.click();
            await new Promise((resolve) => setTimeout(resolve, 120));
        }
        if (step.highlightField) {
            document.querySelector(`[data-smart-field="${step.highlightField}"]`)?.click();
